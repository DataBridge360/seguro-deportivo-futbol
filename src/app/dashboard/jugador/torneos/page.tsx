...
function getBadgeClasses(estado: string) {
  switch (estado) {
    ...
    case 'cancelado': return 'bg-red-100 text-red-600 dark:bg-red-600/30 dark:text-red-400'
    default: return 'bg-slate-100 text-slate-600 dark:bg-slate-600/30 dark:text-slate-400'
  }
}
...
export default function JugadorTorneosPage() {
  ...
  const misInscripcionesTorneo = useMemo(
    () => inscripciones.filter(i => i.torneo_id === selectedTorneo?.id),
    [inscripciones, selectedTorneo],
  )
  ...
  const categoriasDelTorneo = useMemo(() => {
    ...
    equiposTorneo.forEach(e => {
      ...
      else map.set(e.categoria_nombre, { nombre: e.categoria_nombre, count: 1 })
    })
  ...
  const equiposFiltrados = useMemo(() => {
    ...
    return [...result].sort((a, b) => {
      const aEs = misIds.has(a.id) ? 0 : 1
      const bEs = misIds.has(b.id) ? 0 : 1
      return aEs - bEs
    })
  }, [equiposTorneo, categoriaTab, busqueda, misInscripcionesTorneo])

  // ─── Sort torneos ─────────────────────────────────────────────────────────────
  const estadoOrden: Record<string, number> = { en_curso: 0, proximo: 1, finalizado: 2, cancelado: 3 }
  const torneosOrdenados = [...torneos].sort(
    (a, b) => (estadoOrden[calcularEstado(a)] ?? 9) - (estadoOrden[calcularEstado(b)] ?? 9)
  )
